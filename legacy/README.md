...
While the Peacock server officially supports Linux, the patcher does not officially support Linux/Proton.
...
- `start.sh` — Shell script that downloads Peacock, Node.js, copies patcher files, and starts the server
- `peacock.service` — Systemd user service unit file
- `WineLaunch.bat` — Windows batch file that launches both the game and PeacockPatcher.exe
- `Hitman 3 (Peacock).desktop` — Desktop entry for launching the script
